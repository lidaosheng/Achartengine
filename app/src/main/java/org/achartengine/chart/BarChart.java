...
public class BarChart extends XYChart {
  ...
  public void drawSeries(Canvas canvas, Paint paint, List<Float> points,
    ...
    for (int i = 0; i < length; i += 2) {
      ...
      if (mType == Type.HEAPED && seriesIndex > 0) {
        System.out.println("------------------------------这是HEAPED类型的Bar----------------------------------");
        ...
        System.out.println("BarChart:--------(x,yAxisValue,y,halfDiffX,seriesNr,SeriesIndex): "+"("+x+","+yAxisValue+","+y+","+halfDiffX+","+seriesNr+","+seriesIndex+")");
  ...
  protected void drawBar(Canvas canvas, float xMin, float yMin, float xMax, float yMax,
    ...
    int scale = mDataset.getSeriesAt(seriesIndex).getScaleNumber();
      System.out.println("(xMin,yMin,xMax,yMax,halfDiffX,seriesNr,seriesIndex,scale):"+xMin+","+yMin+","+xMax+","+yMax+","+halfDiffX+","+seriesNr+","+seriesIndex+","+scale+")");
    if (mType == Type.STACKED || mType == Type.HEAPED) {
  ...
  protected float getHalfDiffX(List<Float> points, int length, int seriesNr) {
    float barWidth = mRenderer.getBarWidth();System.out.println("BarChart:----barWidth:"+barWidth);
    if (barWidth > 0) {
      return barWidth / 2;
    }
    int div = length;System.out.println("BarChart------div:"+div);
    if (length > 2) {
      div = length - 2;
    }
