...
public abstract class XYChart extends AbstractChart {
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    System.out.println("-----绘制:(top,left,bottom,right):"+"("+top+","+left+","+bottom+","+right+")");
    System.out.println("-------背景绘制："+"(x,y,width,height):"+"("+x+","+y+","+width+","+height+")");
    ...
    for (int i = 0; i < sLength; i++) {
      ...
      if (!isMinXSet[scale]) {
        ...
        System.out.println("scale:"+scale+" isMinXSet:"+isMinXSet[scale]+"minX[scale]:"+minX[scale]+" minimumX:"+minimumX);
    ...
    System.out.println("for循环前看看maxScaleNumber:"+maxScaleNumber);
    for (int i = 0; i < maxScaleNumber; i++) {
      if (maxX[i] - minX[i] != 0) {
        xPixelsPerUnit[i] = (right - left) / (maxX[i] - minX[i]);
        System.out.println("maxX,minX:"+maxX[i]+"-"+minX[i]);
      }
      if (maxY[i] - minY[i] != 0) {
        yPixelsPerUnit[i] = (float) ((bottom - top) / (maxY[i] - minY[i]));
        System.out.println("yPixelsPerUnit["+i+"]:"+yPixelsPerUnit[i]+" bottom,top,maxY[i],minY[i]:"+bottom+"-"+top+"-"+maxY[i]+"-"+minX[i]);
      }
      ...
      if (i > 0) {
        ...
        minX[i] = minX[0];
        maxX[i] = maxX[0];
        System.out.println("进了if语句:"+"(i,minX[0],maxX[0])"+minX[0]+"-"+maxX[i]);
      }
    }
    ...
    if (showLabels) {
      List<Double> xLabels = getValidLabels(getXLabels(minX[0], maxX[0], mRenderer.getXLabels()));
      System.out.println("min"+minX[0]+"max"+maxX[0]+" XLables:"+xLabels.toString()+"-----------------------------");
      Map<Integer, List<Double>> allYLabels = getYLabels(minY, maxY, maxScaleNumber);
      System.out.println("------------------------"+allYLabels.toString());
        ...
        for (int i = 0; i < maxScaleNumber; i++) {
          ...
          for (Double location : yTextLabelLocations) {
            if (minY[i] <= location && location <= maxY[i]) {
              ...
              if (or == Orientation.HORIZONTAL) {
                System.out.println("axisAlign:"+axisAlign+"----------------");
